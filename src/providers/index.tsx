 ...
 *
 * `local-report` reads the JSON file written by `npm run scan`
 * (see scanner/cli.ts) — real storage/cleanup/device data, mock for
 * everything not scanned yet. A future `cloud-api` registry will be added
 * here the same way, with no change to any screen, which only ever calls
 * `useProviders()`.
 */
